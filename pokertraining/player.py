...
class Player:
    ...
    def holeCardsCleaned(self):
        ...
        for card in sorted(self.hole,reverse=True):
            if card.value == 10: cards += 'T'
            elif card.value == 11: cards += 'J'
            elif card.value == 12: cards += 'Q'
            elif card.value == 13: cards += 'K'
            elif card.value == 14: cards += 'A'
            else: cards += str(card.value)
    ...
    def playerLog(self):
        return ','.join(["'"+self.holeCardsCleaned()+"'", str(self.bestHandFlop.score), str(self.bestHandTurn.score), str(self.bestHandRiver.score), str(self.win)])
